...
@Repository
class NonSurrogatePrimaryKeyHistoryBlotterRowDtoDao(
    private val jdbcOps: JdbcOps
) {


    private val dtoRowMapper = NonSurrogatePrimaryKeyHistoryBlotterRowDtoRowMapper()


    private val searchModelConverter = AgGridSearchModelConverter(
        NonSurrogatePrimaryKeyHistoryBlotterRowDtoMeta::fieldNameToColumnName,
        NonSurrogatePrimaryKeyHistoryBlotterRowDtoMeta::fieldNameToJdbcType
    )
    ...
    fun search(entityId: DomainId, searchModel: AgGridSearchModel): SearchResultPage<NonSurrogatePrimaryKeyHistoryBlotterRowDto> {
        ...
        val entityIdFilter = "maia.non_surrogate_primary_key_history.id = :entityId"
        ...
        val sqlForTotalCount = """
            select count(*)
            from maia.non_surrogate_primary_key_history
            $whereClause
            """.trimIndent()

        val sqlForPage = """
            select
                maia.non_surrogate_primary_key_history.change_type as changeType,
                maia.non_surrogate_primary_key_history.some_modifiable_string as someModifiableString,
                maia.non_surrogate_primary_key_history.some_string as someString,
                maia.non_surrogate_primary_key_history.version as version
            from maia.non_surrogate_primary_key_history
            $whereClause
            $orderByClause
