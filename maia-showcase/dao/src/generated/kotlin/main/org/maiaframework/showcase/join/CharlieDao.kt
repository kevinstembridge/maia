// This source was generated by the Maia Framework code generator
// Renderer class: class org.maiaframework.gen.renderers.JdbcDaoRenderer

package org.maiaframework.showcase.join

import org.maiaframework.domain.DomainId
import org.maiaframework.domain.EntityClassAndPk
import org.maiaframework.domain.persist.FieldUpdate
import org.maiaframework.jdbc.EntityNotFoundException
import org.maiaframework.jdbc.JdbcOps
import org.maiaframework.jdbc.MaiaRowMapper
import org.maiaframework.jdbc.SqlParams
import org.springframework.data.domain.Pageable
import org.springframework.stereotype.Repository


@Repository
class CharlieDao(
    private val fieldConverter: CharlieEntityFieldConverter,
    private val jdbcOps: JdbcOps
) {


    private val entityRowMapper = CharlieEntityRowMapper()


    private val primaryKeyRowMapper = MaiaRowMapper { rsa -> rsa.readDomainId("id") }


    private val fetchForEditDtoRowMapper = CharlieFetchForEditDtoRowMapper()


    fun insert(entity: CharlieEntity) {

        jdbcOps.update(
            """
            insert into maia.charlie (
                bravo_id,
                created_timestamp_utc,
                id,
                some_int,
                some_string
            ) values (
                :bravoId,
                :createdTimestampUtc,
                :id,
                :someInt,
                :someString
            )
            """.trimIndent(),
            SqlParams().apply {
                addValue("bravoId", entity.bravoId)
                addValue("createdTimestampUtc", entity.createdTimestampUtc)
                addValue("id", entity.id)
                addValue("someInt", entity.someInt)
                addValue("someString", entity.someString)
            }
        )

    }


    fun bulkInsert(entities: List<CharlieEntity>) {

        jdbcOps.batchUpdate(
            """
            insert into maia.charlie (
                bravo_id,
                created_timestamp_utc,
                id,
                some_int,
                some_string
            ) values (
                :bravoId,
                :createdTimestampUtc,
                :id,
                :someInt,
                :someString
            )
            """.trimIndent(),
            entities.map { entity ->
                SqlParams().apply {
                    addValue("bravoId", entity.bravoId)
                    addValue("createdTimestampUtc", entity.createdTimestampUtc)
                    addValue("id", entity.id)
                    addValue("someInt", entity.someInt)
                    addValue("someString", entity.someString)
                }
            }
        )

    }


    fun count(): Long {

        return jdbcOps.queryForLong(
            "select count(*) from maia.charlie",
            SqlParams()
        )

    }


    fun count(filter: CharlieEntityFilter): Long {

        val whereClause = filter.whereClause(this.fieldConverter)
        val sqlParams = SqlParams()

        filter.populateSqlParams(sqlParams)

        return jdbcOps.queryForLong(
            """
            select count(*) from maia.charlie
            where $whereClause
            """.trimIndent(),
            sqlParams
        )

    }


    @Throws(EntityNotFoundException::class)
    fun findByPrimaryKey(id: DomainId): CharlieEntity {

        return findByPrimaryKeyOrNull(id)
            ?: throw EntityNotFoundException(
                EntityClassAndPk(
                    CharlieEntity::class.java,
                    mapOf(
                        "id" to id,
                    )
                ),
                CharlieEntityMeta.TABLE_NAME
            )

    }


    fun findByPrimaryKeyOrNull(id: DomainId): CharlieEntity? {

        return jdbcOps.queryForList(
            "select * from maia.charlie where id = :id",
            SqlParams().apply {
            addValue("id", id)
            },
            this.entityRowMapper
        ).firstOrNull()

    }


    fun existsByPrimaryKey(id: DomainId): Boolean {

        val count = jdbcOps.queryForInt(
            "select count(*) from maia.charlie where id = :id",
            SqlParams().apply {
                addValue("id", id)
           }
        )
       
        return count > 0
       
    }

    fun findAllBy(filter: CharlieEntityFilter): List<CharlieEntity> {

        val whereClause = filter.whereClause(this.fieldConverter)
        val sqlParams = SqlParams()

        filter.populateSqlParams(sqlParams)

        return this.jdbcOps.queryForList(
            "select * from maia.charlie where $whereClause",
            sqlParams,
            this.entityRowMapper
        )

    }


    fun findPrimaryKeysAsSequence(filter: CharlieEntityFilter): Sequence<DomainId> {

        val whereClause = filter.whereClause(this.fieldConverter)
        val sqlParams = SqlParams()

        filter.populateSqlParams(sqlParams)

        return this.jdbcOps.queryForSequence(
            "select id from maia.charlie where $whereClause",
            sqlParams,
            { rsa -> rsa.readDomainId("id") }
        )

    }


    fun findAllPrimaryKeysAsSequence(): Sequence<DomainId> {

        return this.jdbcOps.queryForSequence(
            "select id from maia.charlie;",
            SqlParams(),
            { rsa -> rsa.readDomainId("id") }
        )

    }


    fun findAllBy(filter: CharlieEntityFilter, pageable: Pageable): List<CharlieEntity> {

        val whereClause = filter.whereClause(this.fieldConverter)
        val orderByClause = orderByClauseFor(pageable)
        val limitClause = limitClauseFor(pageable)
        val offsetClause = offsetClauseFor(pageable)

        val sqlParams = SqlParams()

        filter.populateSqlParams(sqlParams)

        return this.jdbcOps.queryForList(
            "select * from maia.charlie where $whereClause $orderByClause $limitClause $offsetClause",
            sqlParams,
            this.entityRowMapper
        )

    }


    private fun orderByClauseFor(pageable: Pageable): String {

        val properties = pageable.sort.map { "${it.property} ${it.direction}" }.joinToString(", ")
        return "ORDER BY $properties"

    }


    private fun limitClauseFor(pageable: Pageable): String {

        return "LIMIT ${pageable.pageSize}"

    }


    private fun offsetClauseFor(pageable: Pageable): String {

        return "OFFSET ${pageable.offset}"

    }


    fun findAllAsSequence(): Sequence<CharlieEntity> {

        return this.jdbcOps.queryForSequence(
            "select * from maia.charlie;",
            SqlParams(),
            this.entityRowMapper,
        )

    }


    fun existsByBravoId(bravoId: DomainId): Boolean {

        val count = jdbcOps.queryForInt(
            """
            select count(*) from maia.charlie
            where bravo_id = :bravoId
            """.trimIndent(),
            SqlParams().apply {
            addValue("bravoId", bravoId)
            }
        )

        return count > 0

    }


    fun fetchForEdit(id: DomainId): CharlieFetchForEditDto {

        return this.jdbcOps.queryForList(
            """
            select
                maia.bravo.id as bravoId,
                maia.bravo.some_string as bravoName,
                charlie.created_timestamp_utc as createdTimestampUtc,
                charlie.id as id,
                charlie.some_int as someInt,
                charlie.some_string as someString
            from maia.charlie
            join maia.bravo
                on maia.bravo.id = charlie.bravo_id
            where charlie.id = :id
            """,
            SqlParams().apply {
                addValue("id", id)
            },
            this.fetchForEditDtoRowMapper
        ).firstOrNull()
            ?: throw EntityNotFoundException(EntityClassAndPk(CharlieEntity::class.java, mapOf("id" to id)), CharlieEntityMeta.TABLE_NAME)

    }


    fun setFields(updaters: List<CharlieEntityUpdater>) {

        updaters.forEach { setFields(it) }

    }


    fun setFields(updater: CharlieEntityUpdater): Int {

        val sql = StringBuilder()
        val sqlParams = SqlParams()

        sql.append("update maia.charlie set ")

        val fieldClauses = updater.fields
            .map { field ->

                addField(field, sqlParams)
                "${field.dbColumnName} = :${field.classFieldName}"

            }.joinToString(", ")

        sql.append(fieldClauses)
        sql.append(" where id = :id")

        sqlParams.addValue("id", updater.id)

        return this.jdbcOps.update(sql.toString(), sqlParams)

    }


    private fun addField(field: FieldUpdate, sqlParams: SqlParams) {

        when (field.classFieldName) {
            "bravoId" -> sqlParams.addValue("bravoId", field.value as DomainId)
        }

    }


    fun deleteByPrimaryKey(id: DomainId): Boolean {

        val existingEntity = findByPrimaryKeyOrNull(id)

        if (existingEntity == null) {
            return false
        }

        val deletedCount = this.jdbcOps.update(
            "delete from maia.charlie where id = :id",
            SqlParams().apply {
                addValue("id", id)
            }
        )

        return deletedCount > 0

    }


    fun removeByPrimaryKey(id: DomainId): CharlieEntity? {

        val found = findByPrimaryKeyOrNull(id)

        if (found != null) {
            deleteByPrimaryKey(id)
        }

        return found

    }


    fun deleteAll() {
        this.jdbcOps.update("delete from maia.charlie")
    }


}

