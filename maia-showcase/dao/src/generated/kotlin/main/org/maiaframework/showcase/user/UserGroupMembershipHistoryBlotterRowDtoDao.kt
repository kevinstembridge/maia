...
@Repository
class UserGroupMembershipHistoryBlotterRowDtoDao(
    private val jdbcOps: JdbcOps
) {


    private val dtoRowMapper = UserGroupMembershipHistoryBlotterRowDtoRowMapper()


    private val searchModelConverter = AgGridSearchModelConverter(
        UserGroupMembershipHistoryBlotterRowDtoMeta::fieldNameToColumnName,
        UserGroupMembershipHistoryBlotterRowDtoMeta::fieldNameToJdbcType
    )


    fun search(entityId: DomainId, searchModel: AgGridSearchModel): SearchResultPage<UserGroupMembershipHistoryBlotterRowDto> {

        val sqlParams = SqlParams()
        sqlParams.addValue("entityId", entityId)
        val entityIdFilter = "maia.user_group_membership_history.id = :entityId"
        ...
        val sqlForTotalCount = """
            select count(*)
            from maia.user_group_membership_history
            $whereClause
            """.trimIndent()

        val sqlForPage = """
            select
                maia.user_group_membership_history.change_type as changeType,
                maia.user_group_membership_history.effective_from as effectiveFrom,
                maia.user_group_membership_history.effective_to as effectiveTo,
                maia.user_group_membership_history.version as version
            from maia.user_group_membership_history
            $whereClause
            $orderByClause
