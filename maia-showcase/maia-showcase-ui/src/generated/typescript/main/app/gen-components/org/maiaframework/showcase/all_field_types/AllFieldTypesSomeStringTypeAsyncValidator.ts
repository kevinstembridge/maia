// This source was generated by the Maia Framework code generator
// Renderer class: class org.maiaframework.gen.renderers.ui.AsyncValidatorRenderer

import {Injectable, inject} from '@angular/core';
import {AbstractControl, AsyncValidator, FormControl, FormGroup, ValidationErrors} from '@angular/forms';
import {AllFieldTypesSomeStringTypeRequestDto} from '@app/gen-components/org/maiaframework/showcase/all_field_types/AllFieldTypesSomeStringTypeRequestDto';
import {AllFieldTypesCrudService} from '@app/gen-components/org/maiaframework/showcase/all_field_types/all-field-types-crud.service';
import {Observable, of} from 'rxjs';
import {catchError, map} from 'rxjs/operators';



@Injectable({
    providedIn: 'root'
})
export class AllFieldTypesSomeStringTypeAsyncValidator implements AsyncValidator {


    private readonly apiService = inject(AllFieldTypesCrudService);


    validate(ctrl: AbstractControl): Observable<ValidationErrors | null> {

        if (ctrl.value === false || ctrl.dirty === false) {
            return of(null);
        }

        const requestBody = {
            someStringType: ctrl.value
        } as AllFieldTypesSomeStringTypeRequestDto;

        return this.apiService.existsBySomeStringType(requestBody).pipe(
            map(response => (response.invalid ? { message: response.message, notUnique: true } : null)),
            catchError(() => of(null))
        );

    }


}

