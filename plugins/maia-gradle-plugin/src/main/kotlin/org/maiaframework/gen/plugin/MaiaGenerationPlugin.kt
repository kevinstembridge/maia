...
abstract class MaiaGenerationPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        `register the cleanMaiaGeneration task`(project, extension)
    ...
    private fun `register the cleanMaiaGeneration task`(
        project: Project,
        extension: MaiaGenerationExtension
    ) {

        // A dedicated task rather than hooking the `clean` lifecycle task: registering these
        // directories against `clean`'s delete-set makes Gradle treat them as owned build
        // outputs, and it will proactively delete them at the start of ANY build (even an
        // unrelated one, in an unrelated project) where `maiaGeneration` isn't part of that
        // build's task graph.
        project.tasks.register("cleanMaiaGeneration", Delete::class.java) {

            description = "Deletes the source code generated by the maiaGeneration task."
            group = "build"
